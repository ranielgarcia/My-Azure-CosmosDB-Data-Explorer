...
![Cosmos DB Data Explorer — Dark Mode](docs/Dark-Mode.png)
![Cosmos DB Data Explorer — Light Mode](docs/Light-Mode.png)
...
Tabs are **auto‑saved** (debounced, ~1 s) to `server/data/tabs.json` and rehydrated on the next
page load — so your session survives a browser refresh.
...
- Queries are saved server‑side to `server/data/saved-queries.json` (UUID, name ≤ 120 chars, query
  ≤ 100 KB, newest‑first). Atomic writes with a serialized write chain.
